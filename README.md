...
正式发布使用 release 包。当前 release 构建已启用 R8 压缩、资源裁剪和 debug 签名配置，生成文件位于：

```text
app/build/outputs/apk/release/JlenVideo-版本号-release.apk
```

客户端固定服务地址使用运行时解码，避免在 release 包中直接暴露主要接口地址。更新版本时，应用内下载链接应指向 GitHub Release 中的 release APK。

加固建议采用稳妥策略：

- 保留 R8 与资源裁剪作为基础优化。
- 保持固定地址运行时解码。
- 优先补充签名校验、敏感日志过滤和接口侧权限校验。
- 第三方加固应先单独产出测试包，在主流机型上验证登录、搜索、追剧、公告、更新和播放流程。
- 不建议未验证前直接把强加固包作为正式发布包，避免影响播放器、WebView、反射序列化或崩溃日志排查。
