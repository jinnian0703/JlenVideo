...
- [检查更新说明](#检查更新说明)
- [发布建议](#发布建议)
- [2.1.0.2 版本说明](#2102-版本说明)
- [2.1.0.1 版本说明](#2101-版本说明)
...
`2.1.0.2` 主要用于补充最近一轮播放器交互优化，并为片库增加分类筛选能力。

- 播放器支持双击暂停与双击恢复播放，单击与双击手势不再互相打架
- 暂停状态下拖动进度条后会保持暂停，中间播放按钮不再消失
- 锁定状态下的解锁键增加 3 秒自动隐藏，减少遮挡画面
- 进度条拖动时会在上方显示目标时间与快进快退偏移提示
- 切换线路时会尽量保持当前选集，不再默认跳回第一集
- 片库页增加真实分类筛选，支持按类型、地区、年份、语言、状态、版本筛选

## 2.1.0.1 版本说明

当前仓库版本现已提升为 `2.1.0.1`。

`2.1.0.1` 主要用于整理最近一轮播放器交互修复，并补充首页缓存兼容处理。

- 修复全屏播放器锁定状态下左右锁键显示/隐藏乱跳的问题，点击一次只切换一次
- 修复锁定态下触摸取消事件可能误触发锁键显隐切换的问题
- 放大播放器中间的播放按钮，暂停后恢复播放更容易点按
- 修复旧版首页缓存缺少新分页字段时的恢复问题，遇到不兼容缓存会自动清理并重新拉取
