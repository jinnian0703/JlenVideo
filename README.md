...
`JlenVideo` 是一个基于原生 Android + Jetpack Compose 开发的苹果 CMS 影视客户端，当前版本为 `1.1.0` 正式版。
...
- 修复账号页未登录状态下的 Compose 崩溃问题
- 新增应用内崩溃日志记录、查看、复制与清空能力
- 关于页支持未登录状态直接查看
- 修复注册/找回密码流程中验证码加载导致的异常问题
- 继续保留首页推荐位 `level=1` 的原生解析逻辑
...
本仓库当前发布为 `1.1.0` 正式版，建议后续版本遵循语义化版本：
