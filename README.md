...
`JlenVideo` 是一个基于原生 Android + Jetpack Compose 开发的苹果 CMS 影视客户端，当前版本为 `1.1.1` 正式版。
...
- 修复关于页“查看发布”错误跳转到 GitHub 项目主页的问题
- “查看发布”现在固定打开 GitHub Release 页面
- “前往下载”继续保留 APK 直链下载逻辑
...
- 版本号读取自 Release 的 `tag_name`，默认支持 `v1.1.1` 这种格式，进入 App 后会自动去掉前面的 `v`
...
- `versionName` 要和 GitHub Release 的 `tag_name` 对应，例如本地是 `1.1.1`，Release 标签建议用 `v1.1.1`
...
本仓库当前发布为 `1.1.1` 正式版，建议后续版本遵循语义化版本：
